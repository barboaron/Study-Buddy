...
function filterStudyGroup(studyGroups, filters) {
    let f1,f2,f3,f4;
    let res = [...studyGroups];
    f1 = filters.courseName && filterByCourseName(studyGroups, filters.courseName);
    f1 ? f2 = filters.groupType && filterByGroupType(f1, filters.groupType) :
        f2 = filters.groupType && filterByGroupType(studyGroups, filters.groupType);
    f2 ? f3 =  filters.date && filterByDate(f2, filters.date) :
        (f1 ? f3 = filters.date && filterByDate(f1, filters.date) : filters.date && filterByDate(studyGroups, filters.date) );
    f3 ? f4 = filters.numOfParticipant && filterByNumOfParticipant(f3, filters.numOfParticipant) :
        (f2 ? f4 = filters.numOfParticipant && filterByNumOfParticipant(f2, filters.numOfParticipant) :
         (f1 ? f4 = filters.numOfParticipant && filterByNumOfParticipant(f1, filters.numOfParticipant) :
         f4 = filters.numOfParticipant && filterByNumOfParticipant(studyGroups, filters.numOfParticipant)));
